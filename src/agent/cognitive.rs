    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Nudge::CheckpointReminder {
                ...
                write!(
                    f,
                    "{} — {} tool calls since your last memory write. \
            ...
            Nudge::MemorySearchReminder {
                turns_since_search,
            } => {
                write!(
                    f,
                    "📝 MEMORY REMINDER: You haven't called memory_search in {} turns. \
                     If you're about to answer a question about past work, decisions, \
                     or preferences — search first.",
                    turns_since_search
                )
            }
        }
...
impl CognitiveGuardian {
    ...
    pub fn on_tool_call(&mut self, tool_name: &str) -> bool {
        ...
        // Check if this is a memory search
    ...
    pub fn should_auto_breadcrumb(&self) -> bool {
        ...
        self.tool_calls_since_checkpoint > 0
            && self.tool_calls_since_checkpoint % self.config.auto_breadcrumb_interval == 0
    }
    ...
    pub async fn write_auto_breadcrumb(
        &self,
        workspace: &Workspace,
        session_label: &str,
    ) {
...
fn is_checkpoint_tool(name: &str) -> bool {
    matches!(
        ...
        "workspace_write"
            ...
            | "memory_search"  // Not a write, but indicates memory engagement
    )
}
...
mod tests {
    ...
    fn test_checkpoint_resets_on_workspace_write() {
        ...
        // Counter is 0 because the checkpoint itself doesn't count
        // toward the next cycle (it reset the counter).
    ...
    fn test_auto_breadcrumb_interval() {
        ...
        for i in 1..=25 {
            ...
            if i == 10 || i == 20 {
                ...
                assert!(!guardian.should_auto_breadcrumb(), "should not fire at {}", i);
            }
        }
    }
